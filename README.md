...
| M7 | Inject failure scenarios and record an operational evidence trail (`WorkloadRun`) | CRD, controller and a single-controller driver, tested on envtest; the trail refuses rather than concludes when it has a hole. `hack/m7-evidence-trail.sh` drives a real Pod deletion through it on a throwaway cluster — **written and unrun here**, blocked on a host inotify limit the script now names |
