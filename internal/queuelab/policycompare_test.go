...
func TestOneKnobDiffWorksAcrossDifferentRunIDs(t *testing.T) {
	...
	never, err := BuildFixtures(StudyReclaim, "Never", "tx-1", "runA", "ns")
	if err != nil {
		t.Fatal(err)
	}
	any, err := BuildFixtures(StudyReclaim, "Any", "tx-1", "runB", "ns")
	if err != nil {
		t.Fatal(err)
	}
...
func TestOneKnobDiffCatchesNoDifference(t *testing.T) {
	...
	a, err := BuildFixtures(StudyReclaim, "Any", "tx-1", "r1", "ns")
	if err != nil {
		t.Fatal(err)
	}
	b, err := BuildFixtures(StudyReclaim, "Any", "tx-1", "r1", "ns")
	if err != nil {
		t.Fatal(err)
	}
