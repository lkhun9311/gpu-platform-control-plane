...
| GitHub workflows              | **Written** (`ci.yml`, `infra.yml`, `destroy.yml`, `lint.yml`, `test.yml`, `test-e2e.yml`). `destroy.yml` has **run against real AWS credentials and real infrastructure** — it destroyed 96 resources on 2026-09-18 and its residue check ran. The others have not; `infra.yml`'s apply job is deliberately unarmed. `gpu.yml`, the GPU node-group switch this document describes, is **designed only and does not exist** |
